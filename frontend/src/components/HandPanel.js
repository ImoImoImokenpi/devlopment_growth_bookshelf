...
function HandPanel({ isOpen, onClose }) {
    ...
    const { myBookshelf, setMyBookshelf } = useContext(MyBookshelfContext);
    ...
    const addSelectedBooks = async () => {
        ...
        try {
            ...
            // 追加後は手元から除外
            setMyHand((prev) =>
                prev.filter((b) => !selectedIds.includes(b.book_id))
            );

            setMyBookshelf(prev => [
                ...prev,
                ...myHand.filter(b => selectedIds.includes(b.book_id))
            ]);
