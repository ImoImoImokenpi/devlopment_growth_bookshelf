...
function applyPushLogic(positions, targetIsbn, targetRow, targetCol, dragDx, booksPerShelf) {
  ...
  const pushRight = (row, col) => {
    ...
    if (pushRight(row, pos.y + 1)) { positions.set(isbn, { x: pos.x, y: pos.y + 1 }); return true; }
    return false;
  };
  const pushLeft = (row, col) => {
    ...
    if (pushLeft(row, pos.y - 1)) { positions.set(isbn, { x: pos.x, y: pos.y - 1 }); return true; }
    return false;
  };
  return dragDx >= 0
...
function ShelfView() {
  ...
  const getGridPos = useCallback(
    (pxX, pxY) => ({
      row: Math.max(0, Math.min(Math.floor((pxY - FRAME_THICKNESS) / (unitShelfHeight + FRAME_THICKNESS)), shelfCount - 1)),
      col: Math.max(0, Math.min(Math.floor((pxX - FRAME_THICKNESS) / BOOK_WIDTH), currentBooksPerShelf - 1)),
    }),
  ...
  const handleCancelSelection = useCallback(() => {
    setIsModalOpen(false); setMeaning(""); setSelectedIsbns([]);
  }, []);

  const handleRemoveRow = async () => {
    try { await removeShelfRow(); }
    catch (err) { alert(err.message || "その段には本があるため削除できません"); }
  };
  ...
  const handleSyncLayout = useCallback(async (layoutData) => {
    ...
    } catch (err) { console.error("Layout sync failed:", err); }
  }, []);
  ...
  useEffect(() => {
    ...
    const createPattern = (id, url, size) => {
      defs.append("pattern").attr("id", id).attr("patternUnits", "userSpaceOnUse")
        .attr("width", size).attr("height", size)
        .append("image").attr("href", url).attr("width", size).attr("height", size)
    ...
    for (let i = 0; i <= shelfCount; i++) {
      ...
      svg.append("rect").attr("x", 0).attr("y", yPos)
        .attr("width", WIDTH).attr("height", FRAME_THICKNESS).attr("fill", "url(#woodPattern)");
    }
    [0, WIDTH - FRAME_THICKNESS].forEach((x) => {
      svg.append("rect").attr("x", x).attr("y", 0)
        .attr("width", FRAME_THICKNESS).attr("height", HEIGHT).attr("fill", "url(#woodPattern)");
    });

    // ガイド矩形
    const guide = svg.append("rect")
      .attr("width", BOOK_WIDTH).attr("height", BOOK_HEIGHT)
      .attr("fill", "rgba(255,255,255,0.2)").attr("stroke", "#fff")
      .attr("stroke-width", 2).attr("stroke-dasharray", "8,4")
      .style("visibility", "hidden").attr("rx", 6);

    // ハイライト更新関数
    const updateHighlights = (isbns) => {
      svg.selectAll(".book-highlight").remove();
      if (!isbns.length) return;
      booksWithPosition.filter((b) => isbns.includes(b.isbn)).forEach((b) => {
        svg.append("rect").attr("class", "book-highlight")
          .attr("x", b.x).attr("y", b.y)
          .attr("width", BOOK_WIDTH).attr("height", BOOK_HEIGHT)
          .attr("rx", 4).attr("fill", "rgba(100,180,255,0.35)")
          .attr("stroke", "#64b4ff").attr("stroke-width", 2)
          .style("pointer-events", "none");
      });
    };

    // ラバーバンド選択
    const rubberband = svg.append("rect")
      .attr("fill", "rgba(100,180,255,0.15)").attr("stroke", "#64b4ff")
      .attr("stroke-width", 1.5).attr("stroke-dasharray", "6,3").attr("rx", 4)
      .style("visibility", "hidden").style("pointer-events", "none");

    let rbStartX = 0, rbStartY = 0;
    const rubberbandDrag = d3.drag()
      .filter((event) => !event.target.classList.contains("book-group"))
      .on("start", function (event) {
        if (isModalOpen) return;
        const [mx, my] = d3.pointer(event, svgRef.current);
        rbStartX = mx; rbStartY = my;
        rubberband.attr("x", mx).attr("y", my).attr("width", 0).attr("height", 0).style("visibility", "visible");
      })
      .on("drag", function (event) {
        if (isModalOpen) return;
        const [mx, my] = d3.pointer(event, svgRef.current);
        const rx = Math.min(mx, rbStartX), ry = Math.min(my, rbStartY);
        const rw = Math.abs(mx - rbStartX), rh = Math.abs(my - rbStartY);
        rubberband.attr("x", rx).attr("y", ry).attr("width", rw).attr("height", rh);
        const hit = booksWithPosition.filter((b) => b.x < rx+rw && b.x+BOOK_WIDTH > rx && b.y < ry+rh && b.y+BOOK_HEIGHT > ry).map((b) => b.isbn);
      ...
      .on("end", function (event) {
        ...
        const rx = Math.min(mx, rbStartX), ry = Math.min(my, rbStartY);
        const rw = Math.abs(mx - rbStartX), rh = Math.abs(my - rbStartY);
        if (rw < 5 && rh < 5) { updateHighlights([]); return; }
        const hit = booksWithPosition.filter((b) => b.x < rx+rw && b.x+BOOK_WIDTH > rx && b.y < ry+rh && b.y+BOOK_HEIGHT > ry).map((b) => b.isbn);
        if (hit.length > 0) { setSelectedIsbns(hit); setIsModalOpen(true); }
    ...
    const dragHandler = d3.drag()
      .on("start", function (event) {
        ...
        me.attr("data-offsetX", event.x - parseFloat(me.attr("x")))
          .attr("data-offsetY", event.y - parseFloat(me.attr("y")));
      ...
      .on("drag", function (event, d) {
        ...
        const virtualPositions = new Map(
          booksWithPosition.filter((b) => b.isbn !== d.isbn).map((b) => [b.isbn, { x: b.gridRow, y: b.gridCol }])
        );
        const canPush = applyPushLogic(virtualPositions, d.isbn, target.row, target.col, event.dx, currentBooksPerShelf);
        if (canPush) {
          const snap = getPhysPos(target.row, target.col);
          guide.attr("x", snap.x).attr("y", snap.y).style("visibility", "visible");
          svg.selectAll(".book-group").filter((other) => other.isbn !== d.isbn).each(function (other) {
            const vPos = virtualPositions.get(other.isbn);
            const phys = getPhysPos(vPos.x, vPos.y);
            d3.select(this).interrupt().transition().duration(200).ease(d3.easeCubicOut).attr("x", phys.x).attr("y", phys.y);
          });
        } else guide.style("visibility", "hidden");
      })
      .on("end", async function (event, d) {
        const me = d3.select(this);
        const target = getGridPos(+me.attr("x") + BOOK_WIDTH / 2, +me.attr("y") + BOOK_HEIGHT / 2);
        const positionMap = new Map(booksWithPosition.map((b) => [b.isbn, { x: b.gridRow, y: b.gridCol }]));
        const pushed = applyPushLogic(positionMap, d.isbn, target.row, target.col, event.dx, currentBooksPerShelf);
        if (pushed) positionMap.set(d.isbn, { x: target.row, y: target.col });
        const nextLayout = Array.from(positionMap.entries()).map(([isbn, pos]) => ({ isbn, x: pos.x, y: pos.y }));
      ...
      <div style={toolbarStyle}>
        <div style={toolGroupStyle}>
          ...
          <div style={{ display: "flex", gap: "8px" }}>
            ...
            <button onClick={() => updateShelfLayout(parseInt(inputValue))} style={primaryButtonStyle}>適用</button>
          </div>
        </div>
        ...
        <div style={toolGroupStyle}>
          ...
          <div style={{ display: "flex", gap: "10px" }}>
            <button onClick={addShelfRow} style={secondaryButtonStyle}>+ 段を追加</button>
            <button onClick={handleRemoveRow} style={dangerButtonStyle}>− 段を削除</button>
          </div>
        </div>
      </div>
      ...
      {isModalOpen && (
        <div style={{ ...modalOverlayStyle, alignItems: "flex-start", paddingTop: `${modalTopOffset}px` }}>
          <div style={modalContentStyle}>
            ...
            <p style={{ color: "#666", fontSize: "14px", marginBottom: "20px" }}>本の「意味」を入力してください。</p>
            <input
              autoFocus type="text" value={meaning}
              onChange={(e) => setMeaning(e.target.value)}
              placeholder="例: プログラミングの基礎"
              style={modalInputStyle}
            />
            <div style={{ display: "flex", gap: "12px", marginTop: "25px", justifyContent: "flex-end" }}>
              <button onClick={handleCancelSelection} style={secondaryButtonStyle}>キャンセル</button>
              <button onClick={handleSaveMeaning} style={primaryButtonStyle}>保存する</button>
            </div>
          </div>
        </div>
      )}
...
const containerStyle = {
  ...
  alignItems: "center", backgroundColor: "#f4f1ea",
  minHeight: "100vh", fontFamily: "'Inter', sans-serif",
};
const toolbarStyle = {
  display: "flex", alignItems: "center", gap: "30px",
  padding: "15px 30px", backgroundColor: "rgba(255,255,255,0.8)",
...
const inputStyle = {
  ...
  border: "1px solid #ddd", textAlign: "center", fontSize: "16px", fontWeight: "600",
};
...
const modalContentStyle = {
  ...
  width: "420px", boxShadow: "0 25px 50px rgba(0,0,0,0.3)",
};
const modalInputStyle = {
  width: "100%", padding: "15px", borderRadius: "12px",
  border: "2px solid #eee", fontSize: "16px", outline: "none", boxSizing: "border-box",
};
